...
TIMEOUT_SECONDS = 90
OUTPUT_DIR = Path("bench_c_outputs")
...
def call_claude_reconstruct(gemini_pgn: str) -> Tuple[str, str]:
...
"""
    ...
    save_text(
        OUTPUT_DIR / "claude_response_raw.json",
        json.dumps(data, ensure_ascii=False, indent=2),
    )
...
def main() -> int:
    ...
    if len(sys.argv) < 2:
        print("Uso: python bench_gemini_pipeline_c.py planella_II.jpg")
        return 1
    ...
    # 2) Claude reconstruct
    t1 = time.perf_counter()
    claude_model, claude_raw_text = call_claude_reconstruct(gemini_pgn)
    claude_elapsed = time.perf_counter() - t1
    save_text(OUTPUT_DIR / "claude_raw.txt", claude_raw_text)

    claude_obj = extract_first_json_object(claude_raw_text)
    if not claude_obj or not claude_obj.get("pgn"):
        result = {
            "stage": "claude_reconstruct",
            "provider": "gemini->claude",
            "gemini_model": gemini_model,
            "claude_model": claude_model,
            "gemini_elapsed_sec": gemini_elapsed,
            "claude_elapsed_sec": claude_elapsed,
            "total_elapsed_sec": gemini_elapsed + claude_elapsed,
            "json_ok": False,
            "error": "Claude no devolvió pgn usable",
        }
        save_text(
            OUTPUT_DIR / "summary.json",
            json.dumps(result, ensure_ascii=False, indent=2),
    ...
    repaired_pgn = claude_obj.get("pgn", "")
    ...
    save_text(
        OUTPUT_DIR / "claude_parsed.json",
        json.dumps(claude_obj, ensure_ascii=False, indent=2),
    )
    save_text(
